...
-- Ensure snapshot columns exist (works if you already added them too)
...
-- If quests were deleted earlier, still ensure no nulls remain
update public.quest_completions
set
  quest_title = coalesce(quest_title, 'Quest'),
  category = coalesce(category, 'intellect'),
  difficulty = coalesce(difficulty, 'easy'),
  kind = coalesce(kind, 'oneoff'),
  completed_date = coalesce(completed_date, (completed_at::date))
where
  quest_title is null
  or category is null
...
-- Unique indexes to prevent farming (safe if already exist)
...
-- Replace complete_quest to insert snapshot fields + return completion_id
...
begin
  ...
  -- insert completion with snapshot + uniqueness enforcement via indexes
  begin
